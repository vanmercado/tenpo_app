...
/*
* カスタム投稿用カレンダー
* !!!!!!パーマリンクが%post_id%の場合 or
* パーマリンクが%postname%の場合を設定すること!!!!!!
**/
function get_cpt_calendar($cpt, $initial = true, $echo = true) {
    ...
    if ( $previous ):
        ...
        // パーマリンクが%postname%の場合
        $calendar_output .= "\n\t\t".'<td colspan="3" id="prev"><a href="' . get_bloginfo("url") .'/' .$cptname.'/'.$previous->year.'/'.$pmonth.'" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s'), $wp_locale->get_month($previous->month), date('Y', mktime(0, 0 , 0, $previous->month, 1, $previous->year)))) . '">&laquo; ' . $wp_locale->get_month_abbrev($wp_locale->get_month($previous->month)) . '</a></td>';
    ...
    if ( $next ):
        ...
        // パーマリンクが%postname%の場合
        $calendar_output .= "\n\t\t".'<td colspan="3" id="next"><a href="' . get_bloginfo("url") .'/' .$cptname.'/'.$next->year.'/'.$nmonth.'" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s'), $wp_locale->get_month($next->month), date('Y', mktime(0, 0 , 0, $next->month, 1, $next->year))) ) . '">' . $wp_locale->get_month_abbrev($wp_locale->get_month($next->month)) . ' &raquo;</a></td>';
    else:
...
for ( $day = 1; $day <= $daysinmonth; ++$day ) {
    ...
    // パーマリンクが%postname%の場合
    $calendar_output .= '<a href="' . get_bloginfo("url") .'/' .$cptname.'/'.$thisyear.'/'.$thismonth.'/'.$day2.'" title="' . esc_attr( $ak_titles_for_day[ $day ] ) . "\">$day</a>";
    else
    $calendar_output .= $day;
    $calendar_output .= '</td>';
